...
```{r}
## data from OBIS
obis <- read.csv("../data/OBIS/2025/Occurrence.csv")
obis <- obis %>%
...
## data from GBIF
gbif <- read.delim("../data/GBIF/2025/occurrence.txt")
gbif <- gbif %>%
...
length(unique(gbif_human$decimallongitude)) # 566 unique longitude measurements 
length(unique(gbif_human$decimallatitude)) # 572 unique latitude measurements 
...
Merge datasets 
```{r}
obis_human$coordinates <- paste0(obis_human$decimallongitude, sep = ", ", obis_human$decimallatitude)
length(unique(obis_human$coordinates))

gbif_human$coordinates <- paste0(gbif_human$decimallongitude, sep = ", ", gbif_human$decimallatitude)
length(unique(gbif_human$coordinates))

merge <- full_join(obis_human, gbif_human, by = "coordinates") %>%
  tidyr::drop_na(decimallongitude.x, decimallatitude.x, decimallongitude.y, decimallatitude.y)
